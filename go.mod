module github.com/cutd/go-freeling

go 1.12

require (
	github.com/abiosoft/semaphore v0.0.0-20180811165425-cb737ff681bd
	github.com/advancedlogic/go-freeling v0.0.0-20190816150524-d4c3d0731524
	github.com/cheggaaa/pb v2.0.7+incompatible
	github.com/cutd/GoOse v0.0.0-20191028131041-654c0f2471fe
	github.com/fatih/color v1.7.0
	github.com/fatih/set v0.2.1
	github.com/fluhus/gostuff v0.0.0-20181218190919-fc96623389f4
	github.com/gorilla/mux v1.7.3
	github.com/kdar/factorlog v0.0.0-20140929220826-d5b6afb8b4fe
	github.com/mattn/go-colorable v0.1.4 // indirect
	github.com/mattn/go-isatty v0.0.10 // indirect
	github.com/mgutz/ansi v0.0.0-20170206155736-9520e82c474b // indirect
	github.com/nu7hatch/gouuid v0.0.0-20131221200532-179d4d0c4d8d
	github.com/pelletier/go-toml v1.6.0
	github.com/petar/GoLLRB v0.0.0-20190514000832-33fb24c13b99
	github.com/pmylund/go-cache v2.1.0+incompatible
	github.com/satori/go.uuid v1.2.0
	gopkg.in/VividCortex/ewma.v1 v1.1.1 // indirect
	gopkg.in/cheggaaa/pb.v2 v2.0.7 // indirect
	gopkg.in/fatih/color.v1 v1.7.0 // indirect
)
